...
exports.saveDoctorAppointmentOptions = function(doctor_id, options, callback) {
    if(!doctor_id || !options) {
        ...
        async.map(options, function(option, opCallback){
            if(option.id) {
                opCallback(null, mysql.format(sqlsConfig.updateDoctorAppointmentOption,[option.week_day, option.from_time, option.to_time, option.id]));
            } else {
                opCallback(null, mysql.format(sqlsConfig.insertDoctorAppointmentOption, [doctor_id, option.week_day, option.from_time, option.to_time]));
            }
        }, function(err, sqls){
            if(err) { callback(err);}
            else {
...
exports.saveAppointment = function(appointment, callback) {
    if(!appointment) {
        looger.error('saveAppointment appointment undefined');
        callback(new Error('appointment undefined'));
    } else if(!appointment.options || !appointment.patient_id || !appointment.appointment_date || !appointment.from_time || !appointment.to_time) {
        looger.error('saveAppointment one from appointment properties  undefined');
        callback(new Error('one from appointment properties  undefined'));
    } else if(appointment.options.length == 0) {
        looger.error('saveAppointment no appointment options found');
        callback(new Error('no appointment options found'));
    }
};
