    ...
    if(!user_name || !password) {
        ...
        async.waterfall([
            ...
            function(user_id, cb) {
                    ...
                    processQuery('login', sqlsConfig.getUserToken,[user_id, config.user.tokenExpiredIn], function(err, res){
                        if(err) {
                            logger.error('logger error get token %s', util.inspect(err));
                            cb(err);
                        } else {
                            if(res && res.length == 0) {
                                processQuery('login', sqlsConfig.createToken, [user_id], function(e, token_id_res) {
