...
exports.saveDoctorAppointmentOptions = function(doctor_id, options, callback) {
    if(!doctor_id || !options) {
        ...
        async.map(options, function(option, opCallback){
            if(option.id) {
                opCallback(null, mysql.format(sqlsConfig.updateDoctorAppointmentOption,[option.from_time, option.to_time, option.id]));
            } else {
                opCallback(null, mysql.format(sqlsConfig.insertDoctorAppointmentOption, [doctor_id, option.from_time, option.to_time]));
            }
        }, function(err, sqls){
            if(err) { callback(err);}
            else {
