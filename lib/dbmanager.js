...
exports.insertdocument = function(document, entity_type, entity_id, callback) {
    if(!document) {
    ...
    if(!entity_id || !entity_type) {
        logger.error('insertdocument entity_id or entity_type undefined');
        callback(new Error('entity_id or entity_type undefined'));
        return;
    }
    ...
    pool.getConnection(function(err, connection){
        if(err) {
            logger.error('insertdocument %s', util.inspect(err));
            callback(err);
        } else {
            connection.beginTransaction(function() {
                async.waterfall([
                    function(cb) {
                        connection.query(sqlsConfig.insertDocument, [document.name, document.subject, document.file_url], function(insertErr, results) {
                            if(err) cb(insertErr);
                            else {
                               cb(null, results.insertId);
                            }
                    ...
                    function(doc_id, cb) {
                        connection.query(sqlsConfig.apllyDocumentRelation, [doc_id, entity_type, entity_id], function(relErr, results) {
                            if(err) cb(err);
                            else {
                                cb(null,
                                    {
                                        doc_id : doc_id,
                                        rel_id : results.insertId
                                    });
                            }
                        })
                    }
                ], function(cbErr, results) {
                    if(err) {
                        logger.error('insertdocument %s', util.inspect(err));
                        connection.rollback(function() {
                            callback(cbErr);
